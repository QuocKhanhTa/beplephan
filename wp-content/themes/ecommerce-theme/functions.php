...
add_action('wp_enqueue_scripts', 'register_scripts');
    ...
    function owl_carousel_posts_shortcode($atts) {
        ...
        // Thực hiện truy vấn lấy các bài viết của tác giả 292 và trạng thái publish
        $args = array(
            'post_type' => 'post',  // Lấy bài viết (có thể thay bằng 'product' nếu bạn muốn lấy sản phẩm)
            'posts_per_page' => $limit,
            'post_status' => 'publish', // Chỉ lấy bài viết đã được publish
            'author' => 292, // Lọc theo tác giả có ID là 292
        );
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                // Lấy post_title và post_content
                $title = get_the_title();
                ...
                $output .= '<p>' . wp_trim_words($content, 20) . '...</p>';  // Hiển thị nội dung tóm tắt
                $output .= '</a>';
                $output .= '</div>';
            }
    ...
    function display_products_by_category_shortcode($atts) {
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                // echo '</div>';
                // echo '</div>';
                echo '<div class="product__price--percent" style="background: url(' . get_template_directory_uri() . '/assets/images/icon-giam-gia.png) 50% no-repeat"><p class="product__price--percent-detail">Giảm&nbsp;17%</p></div>';
